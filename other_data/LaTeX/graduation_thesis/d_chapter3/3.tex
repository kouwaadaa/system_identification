...
\ref{sec:steady}小節と\ref{sec:airf_lin}小節の内容をふまえ，式(\ref{eq:F_t})と式(\ref{eq:M_t})も考慮して，線形化した縦の運動方程式をまとめると次のようになる．ただし，式(\ref{eq:w_to_alpha})を用いて，$w$に関する式を$\alpha$に関する式に変換した上で，Table \ref{tb:partial_derivatives}にまとめたとおり各項の係数を略記している．
\begin{equation}
  \left\{
  \begin{align}
